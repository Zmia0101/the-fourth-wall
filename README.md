...
- 本项目代码由 **AI 辅助生成**，人类仅负责需求规划与结果审查，未进行逐行人工逻辑审查，也未做完整的软件测试。发布前作者做过**基础的安全检查**（确认不含恶意系统命令、后门或未经授权的数据外传），但这不能替代专业的安全审计。因此代码仍可能存在未被发现的 Bug 或异常行为。
- 建议在沙箱或测试环境中运行，**不建议用于生产环境**，也不建议用于任何商业用途。
- 项目以 MIT 协议发布，作者不对代码的原创性、完整性或适用性作任何担保。
- 使用者需自行承担使用风险，作者不对因使用本项目而产生的任何直接或间接损失负责。
- **请勿将本项目部署为多人共用的公共服务**，也不要分享 `data/` 目录及其备份：用户的 API Key 以可逆方式加密保存在 `data/` 目录，解密密钥（`data/secret.key`）与数据同处一地，任何能读取该目录的人都可以还原其中保存的 Key。
